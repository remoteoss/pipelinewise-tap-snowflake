...
REQUIRED_CONFIG_KEYS = ["account", "dbname", "user", "warehouse", "tables"]
...
DATETIME_TYPES = set(
    ["datetime", "timestamp", "date", "timestamp_ltz", "timestamp_ntz", "timestamp_tz"]
)
...
def discover_catalog(snowflake_conn, config):
    ...
    for k, cols in itertools.groupby(
        columns, lambda c: (c.table_catalog, c.table_schema, c.table_name)
    ):
        cols = list(cols)
        ...
        if (
            ...
            row_count = (
                table_info[table_catalog][table_schema][table_name].get("row_count", 0)
                or 0
            )
...
def get_streams(snowflake_conn, catalog, config, state):
    ...
    if currently_syncing:
        currently_syncing_stream = list(
            filter(lambda s: s.tap_stream_id == currently_syncing, streams_with_state)
        )
...
def do_sync_incremental(snowflake_conn, catalog_entry, state, columns):
    ...
    if not replication_key:
        raise Exception(
            f"Cannot use INCREMENTAL replication for table ({catalog_entry.stream}) without a replication "
            f"key."
        )
